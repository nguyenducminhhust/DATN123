...
export default function CreateDataProcessUser() {
  ...
  const [dpus] = state.dataprocesscustomerAPI.dataprocesscustomer;
  ...
  const handleEmail = (e) => {
   ...
   let emailusercheck = useremail.useremail1.toLowerCase();
  console.log(emailusercheck);
  const checkemailSubmit = () => {
  //  let emailusercheck = useremail.useremail1.toLowerCase();
    const CheckEmail =  userdata[0].filter((userlist) => userlist.email == useremail.useremail1.toLowerCase() )
...
const upContainerService = async ()=> {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
        ...
        await axios.post("/api/dataprocesscustomers", { ...dpu, images});
...
//      upContainerService();
  ...
  return (
    <>
      ...
      <button type="button" onClick={checkemailSubmit}>Tìm theo Email</button>
